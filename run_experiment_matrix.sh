...
#
# Run from the repo root (same directory as train_patch_network.py):
...
script -qec "$CMD1" /dev/null 2>&1 | tee logs_experiments/17class_gateOn_color_baseline.log

echo "[2/6] 17-class, dilation_gating=on, synthetic color, area_thresholds=20/90/180, early_bias_power=1.5 (tuned)"
# Real hardware finding (cycle_0001, testing/logs/snapshots/mesh_viewer_segmented_20260815_122810/)
...
# 0.772->0.742, mIoU 0.549->0.512 - see logs_experiments/threshold_sweep_081226.csv) - expected,
...
script -qec "$CMD2" /dev/null 2>&1 | tee logs_experiments/17class_gateOn_color_tuned.log
...
# script -qec "$CMD3" /dev/null 2>&1 | tee logs_experiments/17class_gateOn.log
#
...
# script -qec "$CMD4" /dev/null 2>&1 | tee logs_experiments/17class_gateOff.log
#
...
# script -qec "$CMD5" /dev/null 2>&1 | tee logs_experiments/5class_gateOn.log
#
...
# script -qec "$CMD6" /dev/null 2>&1 | tee logs_experiments/5class_gateOff.log

echo "Run complete (color baseline + tuned-threshold tonight). Checkpoints under $CKPT_ROOT/<experiment_version>/, logs under logs_experiments/."
