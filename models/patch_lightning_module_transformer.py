...
from models.patch_dilated_tooth_seg_transformer_network import PatchDilatedToothSegTransformerNetwork

# New, additive-only file - models/patch_lightning_module.py (Component 5) is untouched. Same
# training/validation/optimizer setup, wrapping PatchDilatedToothSegTransformerNetwork
# (models/patch_dilated_tooth_seg_transformer_network.py) instead of the non-transformer network -
# duplicated rather than subclassed, since __init__ constructs self.model inline with no factory
# hook to override cleanly (same "duplicate __init__" precedent as elsewhere in this project, e.g.
# dataset/patch_dataset_whole_tooth.py's own docstring on why it duplicates __getitem__).


class PatchLitDilatedToothSegTransformerNetwork(L.LightningModule):
    def __init__(self, num_classes: int = 17, feature_dim: int = 24, k: int = 32,
                 ...
                 lr_gamma: float = 0.5):
        super().__init__()
        self.model = PatchDilatedToothSegTransformerNetwork(
